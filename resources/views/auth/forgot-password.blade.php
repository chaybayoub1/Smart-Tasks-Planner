    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        body {
            ...
            min-height: 100vh; overflow-y: auto; background: #1e1b4b;
        }
        ...
        .fp-logo {
            ...
            color: #fff; letter-spacing: -.03em; display: flex; align-items: center;
            gap: .3rem; text-decoration: none; user-select: none;
        }
        .fp-logo .dot { color: var(--accent); }

        .fp-card {
            width: 100%; background: rgba(255,255,255,.07); border: 1px solid rgba(255,255,255,.12);
            border-radius: 24px; padding: 2.5rem 2.25rem 2.25rem;
        ...
        .fp-icon {
            width: 60px; height: 60px; border-radius: 16px; margin: 0 auto 1.2rem;
            background: linear-gradient(135deg, rgba(99,102,241,.32) 0%, rgba(79,70,229,.20) 100%);
            border: 1px solid rgba(99,102,241,.38); display: flex; align-items: center;
            justify-content: center; font-size: 1.55rem; color: #a5b4fc;
            box-shadow: 0 8px 28px rgba(99,102,241,.22); animation: iconPulse 3s ease-in-out infinite;
        }
        @keyframes iconPulse {
            0%, 100% { box-shadow: 0 8px 28px rgba(99,102,241,.22); }
            50%       { box-shadow: 0 8px 38px rgba(99,102,241,.46); }
        }
        .fp-card-header h2 { font-family: 'Sora', sans-serif; font-size: 1.65rem; font-weight: 700; color: #fff; letter-spacing: -.02em; margin-bottom: .45rem; }
        .fp-card-header p { color: rgba(255,255,255,.45); font-size: .875rem; line-height: 1.6; max-width: 300px; margin: 0 auto; }

        .status-msg {
            background: rgba(99,102,241,.15); border: 1px solid rgba(99,102,241,.32);
            border-radius: 10px; padding: .7rem 1rem; color: #a5b4fc; font-size: .82rem;
            line-height: 1.5; margin-bottom: 1.25rem; display: flex; align-items: flex-start; gap: .5rem;
        }
        ...
        .input-group-st input[type="email"] { width: 100%; padding: .95rem 1.2rem .95rem 2.8rem; background: rgba(255,255,255,.08); border: 1px solid rgba(255,255,255,.12); border-radius: 12px; color: #fff; font-size: .95rem; font-family: 'Inter', sans-serif; outline: none; transition: border-color .2s, background .2s, box-shadow .2s; }
        ...
        .btn-primary-st {
            ...
            border: none; border-radius: 12px; color: #fff; font-family: 'Sora', sans-serif;
        ...
        .btn-primary-st:disabled { opacity: .6; cursor: not-allowed; transform: none; }
...
<div class="fp-center">
    ...
    <div class="fp-card">
        ...
        <form method="POST" action="{{ route('password.email') }}" id="fpForm">
            ...
            <button type="submit" class="btn-primary-st" id="submitBtn">
